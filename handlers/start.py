...
async def command_start_handler(message: Message, bot: Bot, state: FSMContext) -> None:
    ...
    # Получаем или создаем запись пользователя (БЕЗ автоматической активации пробного периода)
    user_data = get_user(user_id)
    
    # Проверяем статус пользователя
    from core.subscription_checker import is_trial_active, is_subscription_active
    ...
    # Показываем кнопку "Начать 3 дня бесплатно" только если пробный период еще НЕ был активирован
    if not trial_was_activated:
    ...
    if has_subscription:
        ...
        status_text = "Вы можете начать наш разговор прямо сейчас. Вам доступен бесплатный пробный период на 3 дня с полным функционалом."
