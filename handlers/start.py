    ...
    if is_new_user:
        ...
        try:
            ...
            call_to_action = (
                ...
                "💡 <b>Быстрый старт:</b>\n"
                "• Команда /new_chat — начать беседу с Духовником\n"
...
async def start_chat_handler(query: CallbackQuery, bot: Bot):
    ...
    chat_prompt = (
        ...
        "Также можете использовать команду /new_chat для начала новой беседы."
    )
...
async def new_chat_handler(message: Message, bot: Bot, state: FSMContext):
    """
    Обработчик команды /new_chat для начала новой беседы с Духовником.
    Очищает историю диалога и приглашает к беседе.
    """
    user_id = message.from_user.id
    user_data = get_user(user_id)
    ...
    logging.info(f"Пользователь {user_id} начал новую беседу с Духовником (история очищена)")
    
    from core.content_sender import send_and_delete_previous
    
    text = (
        "✨ <b>Начинаем новую беседу с Духовником!</b>\n\n"
        "История нашего диалога очищена. Я готов выслушать тебя и помочь.\n\n"
        "💬 Расскажи, что тебя волнует? Задай вопрос о вере, Писании, молитве или духовной жизни.\n\n"
        "Просто напиши мне свой вопрос или мысль. 🙏"
    )
