...
from payments.yookassa import create_premium_payment # Импортируем функцию создания платежа
...
async def subscribe_handler(message: Message, bot: Bot, state: FSMContext):
    ...
    Отправляет предложение о подписке и создает платеж через ЮKassa при нажатии на кнопку.
    """
    ...
    try:
        ...
        logger.info(f"Создание платежа через ЮKassa для user_id={user_id}")
        payment_info = await create_premium_payment(
            user_id=user_id,
            description="Premium подписка «Духовник» на 30 дней"
        )
        ...
        builder = InlineKeyboardBuilder()
        builder.button(
            text="Оплатить 299 ₽ на сайте ЮKassa",
        ...
        subscribe_text = (
            ...
            "💳 <b>Оплата откроется в браузере — безопасно и быстро.</b>\n\n"
            "💡 <i>После оплаты используйте команду /check_payment для проверки статуса платежа</i>"
        )
        ...
        logger.info(f"Сообщение с кнопкой оплаты отправлено user_id={user_id}")
