...
async def subscribe_handler(message: Message, bot: Bot, state: FSMContext):
    ...
    Активирует бесплатный период на 90 дней или показывает статус и платные тарифы.
    """
    user_id = message.from_user.id
    ...
    if free_period_start is None:
        ...
        welcome_text = (
            ...
            "Получите <b>полный доступ ко всем функциям бота на 3 месяца БЕСПЛАТНО!</b>\n\n"
            "✨ <b>Что вас ждет:</b>\n"
        ...
        keyboard = InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="🎁 Активировать бесплатно на 3 месяца", callback_data="activate_free_period")]
        ])
        ...
        if days_left > 0:
            ...
            expired_text = (
                ...
                "Спасибо, что были с нами эти 3 месяца! Надеемся, бот принес вам пользу и духовную поддержку.\n\n"
...
async def activate_free_period_handler(callback_query: CallbackQuery, bot: Bot):
    """
    Обработчик активации бесплатного периода на 90 дней.
    """
    ...
    success_text = (
        ...
        f"✅ Вы получили <b>3 месяца бесплатного доступа</b> ко всем функциям бота!\n"
        ...
        "Расскажите близким об этой возможности — пусть они тоже получат 3 месяца бесплатного доступа к духовной поддержке!\n\n"
